...
public class GlassCard implements ActionListener, PropertyChangeListener {
    ...
    private int index;
    /** the parent explorer */
    private GlassExplorer explorer;
    ...
    public GlassButton getGlassButton() {
    	return button;
    }
    ...
    class GlassButton extends CButton {
        ...
        public void paint(Graphics g) {
            ...
            // Set up first layer
            int buttonHeight = this.getHeight() - 0 * 2;
            int buttonWidth = this.getWidth() - 0 * 2;
            int arc = buttonHeight;

            if (this.pressed || this.cardSelected) {
                g2.setPaint(new GradientPaint(0, -buttonHeight, this.buttonColor, 0, buttonHeight, Color.WHITE, false));
                g2.fillRect(0, 1, buttonWidth, buttonHeight);
            } else {
                //paint highlightlayer
                if (this.focus) {
                	Color hc = canvas.getColor();
                	hc = new Color(hc.getRed(),hc.getGreen(),hc.getBlue(),200);
                    g2.setColor(hc);
                }
                else {
                ...
                g2.drawRect(0, 0, buttonWidth, buttonHeight);
            }
            ...
            if (text != null && buttonHeight > 4) {
                ...
                float y = (float) ((this.getHeight() / 2) + (textBounds.getHeight() / 2) + 2) - metrics.getDescent();

                g.setColor(Color.black);
                for (int i = 0; i < shadowPositionArray.length; i++) {
                    int dx = shadowPositionArray[i][0];
                    int dy = shadowPositionArray[i][1];
                    g2.setColor(new Color(0.35f, 0.35f, 0.35f, shadowColorArray[i]));

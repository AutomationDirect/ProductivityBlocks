...
public abstract class LabelWidget extends JComponent {
    ...
    private CodeBlockLabel codeBlockLabel = new CodeBlockLabel();
    /** TextField that is visable iff editingText is true */
    private final BlockLabelTextField textField = new BlockLabelTextField();
    //CHANGED: add text editor for code injection blocks
    private final BlockCodeEditorScrollPane codeEditorScrollPane;
    private final JButton doneButton = new JButton("Done");
    private final JPanel codeBlockPanel = new JPanel();
    public boolean isCodeBlock = false;
     ...
     *
     */
    public LabelWidget(String initLabelText, Color fieldColor, Color tooltipBackground, boolean isCodeEditor) {
        this.isCodeBlock = isCodeEditor;
        ...
        codeEditorScrollPane = new BlockCodeEditorScrollPane(new BlockCodeEditor());
        if(isCodeEditor) 
        {
            codeEditorScrollPane.codeEditor.setForeground(Color.BLACK);//white text
            codeEditorScrollPane.codeEditor.setBackground(Color.WHITE);//background matching block color
            codeEditorScrollPane.codeEditor.setCaretColor(Color.BLACK);//white caret
            codeEditorScrollPane.codeEditor.setSelectionColor(Color.BLUE);//black highlight
            codeEditorScrollPane.codeEditor.setSelectedTextColor(Color.WHITE);//white text when highlighted
            codeEditorScrollPane.codeEditor.setBorder(textFieldBorder);
            codeEditorScrollPane.codeEditor.setMargin(textFieldBorder.getBorderInsets(codeEditorScrollPane.codeEditor));
            codeBlockLabel.setForeground(Color.WHITE);
            codeBlockLabel.setBackground(fieldColor);
    		doneButton.setBorderPainted(false);
    		doneButton.setFocusPainted(false);
    		doneButton.setContentAreaFilled(true);
    		doneButton.setForeground(Color.DARK_GRAY);
    		doneButton.setBackground(fieldColor);
    		...
    		codeBlockPanel.add(codeEditorScrollPane);
    		JPanel panel = new JPanel(new FlowLayout(FlowLayout.CENTER,0,0));
    		panel.add(doneButton);
    		codeBlockPanel.add(panel);
    		doneButton.setPreferredSize(new Dimension(300,20));
    		doneButton.setMinimumSize(new Dimension(200,20));
    		codeBlockPanel.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
        }
    }
    ...
    public String getRawCodeText() {
    	return codeEditorScrollPane.codeEditor.getText();
    }
    
    public void setRawCodeText(String text) {
    	codeEditorScrollPane.codeEditor.setText(text);
    }
    public void addKeyListenerToTextField(KeyListener l) {
        if(!isCodeBlock)
        	textField.addKeyListener(l);
        else
        	codeEditorScrollPane.codeEditor.addKeyListener(l);
    }
    ...
    public void addMouseListenerToLabel(MouseListener l) {
        if(!isCodeBlock)
        	...
        	codeBlockLabel.addMouseListener(l);
    }
    ...
    public void addMouseMotionListenerToLabel(MouseMotionListener l) {
        if(!isCodeBlock)
        	...
        	codeBlockLabel.addMouseMotionListener(l);
    }
    ...
    public void setEditingState(boolean editing) {
        if (editing) {
            ...
            if(!isCodeBlock) {
            	...
            	this.add(codeBlockPanel);
            	//this.add(codeEditorScrollPane);
            	suggestEditable(false);
            	codeEditorScrollPane.codeEditor.grabFocus();
            }
            ...
            if (editingText) {
            	if(!isCodeBlock) {
            		...
            		setText(codeEditorScrollPane.codeEditor.getText());
            	}
            }
            editingText = false;
        }
    ...
    public void assignToolTipToLabel(String text) {
    	if(!isCodeBlock) {
    		...
    		this.codeBlockLabel.setToolTipText(text);
    	}
    }
    /**
     * getText
    ...
    public String getText() {
    	if(!isCodeBlock)
    		...
    		return codeBlockLabel.getText().trim();
    	}
    }
    ...
    public void updateLabelText(String text) {
        ...
        {
       	 	codeBlockLabel.setText("C++ Code Block");
        }
        ...
        if(!isCodeBlock)
        	...
        	this.add(codeBlockLabel ,BorderLayout.CENTER);
        	//this.add(codeEditorScrollPane, BorderLayout.SOUTH);
        }
        if (hasSiblings) {
    ...
    private void updateDimensions() {
    	if(!isCodeBlock) {
    		...
    		//do same for text pane
    		Dimension updatedDimension = new Dimension(
    				codeEditorScrollPane.getPreferredSize().width,
    				codeEditorScrollPane.getPreferredSize().height);
    		if(this.hasSiblings) {
    			updatedDimension.width += LabelWidget.DROP_DOWN_MENU_WIDTH;
    		}
    		codeEditorScrollPane.setSize(updatedDimension);
    		codeBlockLabel.setSize(updatedDimension);
    		this.setSize(updatedDimension);
    		this.fireDimensionsChanged(this.getSize());
    	}
    }
    ...
    public void highlightText() {
    	if(!isCodeBlock)
    		...
    		this.codeEditorScrollPane.codeEditor.setSelectionStart(0);
    }
    ...
    public boolean canProcessNegativeSign() {
    	...
    	else //codeEditor can have negative sign
    		return true;
    }
    ...
    private class BlockCodeEditorScrollPane extends CGlassScrollPane {
    	
    	/**
		 * 
		 */
		private static final long serialVersionUID = 1L;
		public BlockCodeEditor codeEditor;
    	private BlockCodeEditorScrollPane(BlockCodeEditor editor) {
    		super(editor, ScrollPolicy.VERTICAL_BAR_ALWAYS,
    ...
    /*Text box to support multi-line code injection*/
    private class BlockCodeEditor extends JTextPane implements MouseListener, DocumentListener, FocusListener {
    	
    	/**
		 * 
		 */
		private static final long serialVersionUID = 873847239235L;
		private BlockCodeEditor() {
    		this.addFocusListener(this);
    		this.getDocument().addDocumentListener(this);
    		this.addMouseListener(this);
    		this.setFocusTraversalKeysEnabled(false);
    ...
    private class LabelMenu extends JPanel implements MouseListener, MouseMotionListener {
        ...
        public void paint(Graphics g) {
            ...
            if (isFocused) {
    			...
    			g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_LCD_HRGB); //CHANGED
    ...
    private class ShadowLabel extends JLabel implements MouseListener, MouseMotionListener {
        ...
        private final float[] shadowColorArray = {0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0.2f, 0, 0};
        ...
        public void paint(Graphics g) {
            ...
            for (int i = 0; i < shadowPositionArray.length; i++) {
                ...
                g2.setColor(new Color(0f, 0f, 0f, shadowColorArray[i]));
                g2.drawString(this.getText(), (int) ((4 + dx) * offsetSize), this.getHeight() + (int) ((dy - 6) * offsetSize)); //CHANGED
            }
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if ((e.getClickCount() == 1) && isEditable) {
                ...
                if(isCodeBlock) {
                	codeEditorScrollPane.codeEditor.setSelectionStart(0);
                }
                else {
    ...
    private class CodeBlockLabel extends JLabel implements MouseListener, MouseMotionListener {
        ...
        public void paint(Graphics g) {
            ...
            for (int i = 0; i < shadowPositionArray.length; i++) {
                int dx = shadowPositionArray[i][0];
                int dy = shadowPositionArray[i][1];
                g2.setColor(new Color(0.4f, 0.4f, 0.4f, shadowColorArray[i]));
                g2.setColor(new Color(0f,0f,0f, shadowColorArray[i]));
                g2.drawString(this.getText(), (int) ((4 + dx) * offsetSize), this.getHeight() + (int) ((dy - 6) * offsetSize)); //CHANGED
            }

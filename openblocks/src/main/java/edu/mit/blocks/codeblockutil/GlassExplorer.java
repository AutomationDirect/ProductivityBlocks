...
public class GlassExplorer extends JPanel implements Explorer, FocusListener {
    ...
    public GlassExplorer(Workspace workspace) {
        ...
        retardedPane.setBackground(color);
        ...
        canvasPane.setBackground(color);
    ...
    public void selectCanvas(int index) {
        if ((!timer.timer.isRunning()) || (this.selectedCanvasIndex != index)) {
            if (index >= 0 && index < drawers.size()) {
                ...
                timer.expand();
                canvasPane.setBounds(buttonPane.getWidth(), buttonPane.getY(), 0, 0);
                for (GlassCard otherCard : drawers) {
                    otherCard.getGlassButton().cardSelected = false;
                    otherCard.getGlassButton().repaint();
                }
                card.getGlassButton().cardSelected = true;
                this.requestFocus();
            }
        }
    }
    ...
    public void resetButtonHighlighting() {
        for (GlassCard card : drawers) {
            card.getGlassButton().cardSelected = false;
            card.getGlassButton().repaint();
        }
    }

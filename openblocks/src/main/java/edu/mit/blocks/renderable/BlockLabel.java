...
public class BlockLabel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    public final static Font blockFontSmall_Bold = new Font("Dialog", Font.PLAIN, 8); //CHANGED  Dialog, DialogInput,Monospaced, Serif, or SansSerif.
    public final static Font blockFontMedium_Bold = new Font("Dialog", Font.PLAIN, 11);
    public final static Font blockFontLarge_Bold = new Font("Dialog", Font.BOLD, 13);
    public final static Font blockFontSmall_Plain = new Font("Dialog", Font.PLAIN, 7);
    public final static Font blockFontMedium_Plain = new Font("Dialog", Font.PLAIN, 10);
    public final static Font blockFontLarge_Plain = new Font("Dialog", Font.PLAIN, 12);
    ...
    public BlockLabel(Workspace workspace, String initLabelText, BlockLabel.Type labelType, boolean isEditable, long blockID, boolean hasComboPopup, Color tooltipBackground) {
        ...
        isCodeBlock = workspace.getEnv().getGenusWithName(workspace.getEnv().getBlock(blockID).getGenusName()).isRawCodeBlock();
        widget = new LabelWidget(initLabelText, workspace.getEnv().getBlock(blockID).getColor(), tooltipBackground, isCodeBlock) {
        ...
        if(!isCodeBlock) {
        	...
        	rawCodeText = workspace.getEnv().getBlock(blockID).getCodeBlockText();
        	widget.setRawCodeText(rawCodeText);
        }
    ...
    public int getAbstractHeight() {
        return (int) (widget.getHeight() + 0 / zoom);//CHANGED
    }
    ...
    protected void textChanged(String text) {
        if ((this.labelType.equals(BlockLabel.Type.NAME_LABEL) || this.labelType.equals(BlockLabel.Type.PORT_LABEL))
            ...
            if (this.labelType.equals(BlockLabel.Type.NAME_LABEL)) {
                if(this.isCodeBlock) {
                	workspace.getEnv().getBlock(blockID).setCodeBlockText(widget.getRawCodeText());
                }
                else	
    ...
    protected boolean textValid(String text) {
    	//CHANGED: raw text is always valid:
    	if(!isCodeBlock)	
    		return !text.equals("") && BlockUtilities.isLabelValid(workspace, blockID, text);
    	else
    		return true;
    }

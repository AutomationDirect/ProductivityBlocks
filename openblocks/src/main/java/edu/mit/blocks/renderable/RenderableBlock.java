...
public class RenderableBlock extends JComponent implements SearchableElement,
	...
	//CHANGED: Add boolean describing if the block injects raw code:
	...
	private RenderableBlock(Workspace workspace, WorkspaceWidget parent,
		...
		setLayout(null);
		//check if the block injects raw code
		this.isCodeBlock = workspace.getEnv().getGenusWithName(workspace.getEnv().getBlock(blockID).getGenusName()).isRawCodeBlock();
	...
	public boolean getBlockLabelEditingState() {
		return this.blockLabel.editingText();
	}

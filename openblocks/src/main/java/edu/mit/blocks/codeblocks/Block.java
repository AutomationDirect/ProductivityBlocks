...
public class Block implements ISupportMemento {
    ...
    protected Block(Workspace workspace, Long id, String genusName, String label, boolean linkToStubs) {
        ...
        //CHANGED: Check if block accepts raw code.
        this.isCodeBlock = genus.isRawCodeBlock();
    ...
    //CHANGED: Add getters and setters for codeBlockText
    /**
     * Sets the codeBlockText of this
     * @param codeBlockText the desired page label
     */
    public void setCodeBlockText(String text) {
    	this.codeBlockText = text;
    }
    
    /**
     * Gets the codeBlockText of this
     * @param codeBlockText the desired page label
     */
    public String getCodeBlockText() {
    	return this.codeBlockText;
    }
    ...
    public Node getSaveNode(Document document, int x, int y, Node commentNode, boolean isCollapsed) {
    	...
    	blockElement.setAttribute("is-raw-code-block", isCodeBlock ? "yes" : "no");
    	if (hasFocus) {
        ...
        if(isCodeBlock) {
        	Element rawCodeElement = document.createElement("rawCode");
        	rawCodeElement.setTextContent(getCodeBlockText());
        	blockElement.appendChild(rawCodeElement);
        }
    ...
    public static Block loadBlockFrom(Workspace workspace, Node node, HashMap<Long, Long> idMapping){
        ...
        //CHANGED: Add functionality for raw code block
        ...
        if (node.getNodeName().equals("Block")) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                } else if(child.getNodeName().equals("rawCode")) {
                    rawCodeText = child.getTextContent();
            ...
            if(isCodeBlock) {
            	block.isCodeBlock = isCodeBlock;
            }
        	block.setCodeBlockText(rawCodeText);

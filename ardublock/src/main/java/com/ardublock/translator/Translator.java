...
public class Translator
{
	...
	public String genreateHeaderCommand()
	{
		...
		if(!preprocessorCommandSet.isEmpty())
		{
			for (String command:preprocessorCommandSet)
			{
	...
	public void reset()
	{
		...
		preprocessorCommandSet = new LinkedHashSet<String>();
		...
		subProgramCodeSet = new LinkedHashSet<String>();
	...
	public void addPreprocessorCommand(String command) 
	{
		if(!preprocessorCommandSet.contains(command))
		{
			preprocessorCommandSet.add(command);
		}
	}
	public void addSubprogramCommand(String command) 
	{	
		subProgramCodeSet.add(command);
	}
	...
	public Set<RenderableBlock> findSubroutineBlocks() throws SubroutineNameDuplicatedException
	{
		...
		for (RenderableBlock renderableBlock:renderableBlocks)
		{
			...
			if (!block.hasPlug() && (Block.NULL.equals(block.getBeforeBlockID())))
			{
				...
				else if (block.getGenusName().equals("subprogram_block"))
				{
					String functionName = block.getBlockLabel().trim();
					this.addFunctionName(block.getBlockID(), functionName);
					subroutineBlockSet.add(renderableBlock);
